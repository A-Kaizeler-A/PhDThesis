...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{"images/Paper Images/badimagecmap"}
	\caption{}
	\label{fig:badimagecmap}
\end{figure}
...
\comment{For Nuno: Achas que a frase das correlações ficou mais clara assim?}
...
\end{enumerate}
For this dataset, read length was 152 \gls{nt}. Fragment length distributions were stored as probability tables representing the frequency of each observed size. Transcript expression values were first normalised to \gls{tpm}, and then scaled to reflect the sequencing depth of \gls{wc} (1,000,000) or \gls{sg} (100,000) libraries, since \gls{sg}s were described as containing around 10\% of all \gls{rna} in a cell. These values were then used to determine the number of fragmentation events per transcript. For each event, fragment lengths were randomly drawn from the relevant empirical size distribution, and the transcript was iteratively fragmented until no fragment length could be accommodated by the remaining sequence. This procedure preserved both the transcript origin and fragment size of each simulated read. Simulated \glspl{tpm} were then converted back to read counts, maintaining total read depth equivalent to empirical \gls{sg} datasets. This resulted in three simulated WC and three simulated SG samples, allowing us to assess whether subsampling to 10\% of the total reads (approximating the SG fraction) is sufficient to introduce a length-dependent bias in transcript representation.
For this dataset, read length was 152 \gls{nt}. \comment{For Nuno: A francisca queixou-se que não era claro porque mudava para TPMs, e depois voltava para counts. Rescrevi a secccao para tentar deixar mais explicito.}Fragment length distributions were represented as empirical probability tables describing the frequency of each observed fragment size. Transcript abundances were first normalised to \gls{tpm}, providing length-normalised estimates of transcript representation independent of transcript size and library depth. TPM values were then scaled according to the target sequencing depth of either \gls{wc} (1,000,000 reads) or \gls{sg} (100,000 reads) libraries, reflecting reports that \gls{sg}s contain approximately 10\% of total cellular \gls{rna}\cite{khong_stress_2017}. These scaled abundances were used to estimate the number of fragmentation events assigned to each transcript.
\\
\\
...
All analyses were performed on the workstation \textit{lobito} at the \gls{gimm} data centre, which is equipped with 19 \gls{tb} of \gls{ssd} storage, 768 \gls{gb} of RAM, 72 \gls{cpu} threads (\gls{intel} Xeon
...
\\
\\
For example, FastQC reports base quality scores across reads, which can reveal degradation patterns\cite{andrews_fastqc_2010}. In \gls{rnaseq} data, degradation often appears as reduced quality toward the 3' end of reads. In \gls{pe} sequencing, the second read of the pair typically exhibits slightly lower quality because it is sequenced after the first read and therefore accumulates more ``degradation" (Figure 3.1A). 
For example, FastQC reports per-base quality scores across sequencing reads, enabling the identification of systematic declines in sequencing quality\cite{andrews_fastqc_2010} \comment{For Nuno: Aqui, tinhas tipo que nao ficava claro o que era a degradação, e porque e que a qualidade baixava. Como os FastQC dão os quality scores do sequencing, e nao do RNA em si, mudei para "deteriorizacao" e rescrevi a secção.}. In \gls{rnaseq} datasets, reduced base-calling accuracy is commonly observed toward the 3' end of reads as sequencing quality deteriorates over successive sequencing cycles. In paired-end sequencing, the second read of a pair typically exhibits slightly lower quality than the first, since it is generated after an additional round of sequencing chemistry and therefore accumulates more technical noise (Figure 3.1A).
\\
\\
FastQC also identifies overrepresented sequences, which may indicate technical artefacts\cite{andrews_fastqc_2010} (Figure 3.1B). These sequences can correspond to sequencing adapters if adapter trimming was not performed, or to \gls{rrna} fragments if ribosomal depletion during library preparation was inefficient. 
...
\\
\\
\textbf{Sample coverage} (Figure 3.2A) reflects the effective sequencing depth, defined here as the total number of reads assigned to annotated genes in a given sample\cite{costa_uncovering_2010}. To visualise this, cumulative coverage plots were generated by ranking genes by expression level (in raw counts) and computing the cumulative number of reads as progressively more genes are included. In these plots, the X-Axis represents the number of genes (ordered from highest to lowest expression), while the Y-Axis shows the cumulative fraction of total reads. Typically, the curve rises steeply for the most highly expressed genes and then approaches a plateau after approximately 10,000 genes, indicating that additional sequencing depth predominantly captures lowly expressed transcripts rather than substantially increasing the number of detected genes.
\\
\\
\textbf{Sample complexity} (Figure 3.2B) describes the distribution of reads across genes, providing a measure of library diversity\cite{costa_uncovering_2010}. It was estimated by determining the number of genes required to account for \textit{X}\% of the total reads. To compute this, genes were ranked by expression and their cumulative contribution to total reads was calculated until full library was reached. Samples with low complexity reach high thresholds with relatively few genes, indicating that a small subset of highly expressed transcripts dominates the library. Conversely, high-complexity samples require a larger number of genes the same thresholds, reflecting a more even distribution of reads. Low complexity can arise from technical factors, such as poor mapping or library bias, or from biological features, such as the overrepresentation of specific transcripts\cite{costa_uncovering_2010}.
\\
\\
These metrics are particularly useful for identifying outlier or potentially compromised samples. In this context, problematic samples are defined as those displaying substantial deviations in coverage or complexity relative to the cohort, indicative of technical issues such as low sequencing depth, poor mapping efficiency, or overrepresentation of a small subset of transcripts.
\begin{itemize}
...
\item \textbf{Sample complexity}\comment{For Nuno: Aqui disseste que tinha de ser mais especifico. Para evitar mal-entendidos, fui mais simples e falo apenas de technical bias (e nao de biologico), dando o exemplo de mau ribo-depletion} (Figure 3.2B) describes the distribution of reads across genes, providing a measure of library diversity\cite{costa_uncovering_2010}. It was estimated as the number of genes required to account for \textit{Y}\% of total reads, computed by ranking genes by decreasing expression and calculating their cumulative contribution until the full library was accounted for. Samples with low complexity reach high read proportions with relatively few genes, indicating that a small subset of highly expressed transcripts dominates the library; conversely, high-complexity samples require a greater number of genes to reach the same proportions, reflecting a more even read distribution. Low complexity may arise for example, from technical factors such as incomplete \gls{rrna} depletion leading to contamination of the library with highly abundant ribosomal transcripts\cite{costa_uncovering_2010}.
\end{itemize}
...
\\
\\
All of these assessments were performed in an \textit{ad hoc} manner, on a dataset by dataset basis.
All of these assessments were performed in an empirical manner, on a dataset by dataset basis. \comment{For Nuno: Changed the image to linear, instead of log.}

\begin{figure}[H]
	\centering
<<<<<<< Updated upstream
	\includegraphics[width=1\linewidth]{"images/Paper Images/TESE_Fig4.2"}
	\caption[Sample Coverage and Complexity]{\textbf{Sample Coverage and Complexity.} (A) Cumulative read coverage plot showing the distribution of sequencing reads across genes for each sample (\gls{ie}, how many reads are assigned to how many genes). The data correspond to samples from GSE99304\cite{khong_stress_2017}. In this example, differences in coverage appear to be sample-type specific, as \gls{sg} samples consistently exhibit higher coverage than their \gls{wc} counterparts. (B) Sample complexity plot showing the number of genes required to account for \textit{X}\% of the total reads. An horizontal dashed line is shown at 50\% of total reads, to represent half of total library. The data correspond to samples from GSE143413\cite{honda_high-depth_2021}. In this case, two samples display markedly lower complexity, clearly observed at around 75\% of the total reads. As these represent technical control samples in the original study\cite{honda_high-depth_2021}, this pattern is expected. However, it is important to note that direct comparisons involving these samples will inevitably yield numerous apparent differences due solely to their lower complexity.}
=======
...
\\
\\
In this work, normalisation factors were determined using the \gls{tmm} method\cite{robinson_scaling_2010} implemented in \gls{edger}\cite{robinson_edger_2010}. Unlike simple library-size scaling (introduced in the previous paragraph), \gls{tmm} corrects not only for differences in sequencing depth but also for composition biases, which occur when a small number of highly expressed genes dominate the sequencing library\cite{robinson_scaling_2010}. The method estimates scaling factors between samples while excluding genes with extreme expression values, ensuring that the majority of relatively stable genes determine the normalisation factors\cite{robinson_scaling_2010,robinson_edger_2010}. This approach provides robust correction for \gls{rnaseq} datasets and improves the reliability of downstream differential expression analyses\cite{robinson_scaling_2010,robinson_edger_2010} (Figure 3.3C). The filtered read counts were then normalised using these factors and Log2-transformed using the \texttt{\gls{voom}}\cite{law_voom_2014} function from \gls{limma}\cite{ritchie_limma_2015}.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{"images/Paper Images/TESE_Fig4.3"}
	\caption[Filtering and Normalisation]{\textbf{Filtering and Normalisation.} (A) Density plot showing the distribution of summed reads per gene (Log10 scale) before (Left) and after (Right) filtering, in samples from GSE188397\cite{sato_stress_2023}. In this example, the filtering threshold was set at Log10 = 3.25, corresponding to the boundary between the lowly-expressed and other gene populations. (B) Scatter plot comparing two replicates from the same sample type in GSE171792\cite{shen_sexually_2022}; genes as circles, axes given in \gls{cpm}, Spearman’s correlation coefficient $\rho$ and associated test’s p-value on top left corner. After filtering (Right), the samples appear less similar than before filtering (Left). This occurs because many genes with zero or near-zero counts artificially inflate correlation estimates when retained. Even after filtering, the similarity between replicates remains somewhat lower than reported in the original study\cite{shen_sexually_2022}, possibly due to differences in normalisation methods. (C) Box plots showing the distribution of read counts across samples from PRJNA115262153\cite{rajachandran_subcellular_2025} before (left) and after (right) normalisation. Prior to normalisation, noticeable differences between samples are present, reflecting variation in sequencing depth and library composition. These discrepancies are largely resolved after the normalisation step. Colours correspond to sample types. Although this distinction is not relevant for the purpose of this example, colours were kept to illustrate that read count distribution is often sample type related.}
...
\\
\\
\Gls{pca} plots can reveal issues with specific samples, including underlying technical problems such as differences in sample complexity or coverage, as well as sample label misattribution (Figure 3.4C) or poorly behaving replicates (Figure 3.4D).
\\
\\
...
\linebreak
\noindent
For the analysis of this dataset, gene expression was modelled using a linear model including Target, StressState, and their interaction, without an intercept:
\begin{verbatim}
...
\\
\\
In contrast, \gls{pl}-based datasets showed no marked \gls{rna} length bias in \glspl{sg} (Figure 3.6A, Right; Somasekharan \gls{etal} 2020\cite{somasekharan_g3bp1-linked_2020} study shown as example) and exhibited minimal correlation between \gls{sg} gene expression profiles, even under the same stress stimulus (Figure 3.6B).
\\
\\
...
\\
\\
As control, we simulated \gls{rnaseq} using \gls{wc} and \gls{wc} expression data with their respective fragmentation profiles (\gls{ie} how specific transcripts fragment in each sample type), reproducing empirical fragment length distributions(\gls{vi} section 3.1.3\cite{khong_stress_2017}) (Figure 3.9A). Additional simulations in which expression data from one population were paired with the alternate fragmentation profile demonstrated that fragmentation patterns depended only on the input expression, confirming equivalence of \gls{sg} and \gls{wc} fragmentation profiles (Figure 3.9B).
\\
\\
...
\\
\\
To evaluate this possibility, we compared differential gene expression between \gls{sg} and \gls{wc} fractions in \gls{ac4c}- and \gls{m6a}-knockout cells relative to their wild-type counterparts. These comparisons revealed minimal differences overall, particularly for \gls{m6a} (Figure 3.12A). Assuming that the remaining differences are attributable to the corresponding \gls{rna} modifications, the residuals from a linear model predicting transcript sequestration within \glspl{sg} provide a measure of the extent to which these modifications influence sequestration\footnote{It is worth noting that in the original studies, the reported effects of \gls{rna} modifications were likely driven in part by differences already present in \gls{wc} samples, arising from direct comparisons between different sample types. An example illustrating this analytical pitfall is shown in Section 3.2.6. This is not definitive indication that RNA modifications have no effect in SG sequestration, but that if they have, that this effect is not easily distinguishable from the DC bias.}.
\\
\\
After excluding transcripts with fewer than four or five modifications (for \gls{m6a} and \gls{ac4c}, respectively), the magnitude of \gls{sg} sequestration changes correlates with the number of modifications, particularly for \gls{ac4c} (Figure 3.12B).
\\
\\
Simulations incorporating the additional molecular mass introduced by these modifications reproduced this pattern, showing a similar correlation for transcripts containing more than five or six modifications (for \gls{m6a} and \gls{ac4c}, respectively) (Figure 3.12C). 
\\
\\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{"images/Paper Images/Figure4"}
...
\end{figure}
\noindent
Although individual gene enrichment varied substantially across studies, we used gene set enrichment analysis to identify broader regulatory trends. No pathways were consistently enriched or depleted across all \gls{sg} datasets (Figure 3.17A).
\\
\\
...
\\
\\
To investigate this further, we performed \gls{gsea} across all datasets using a gene set containing the thirty-six mitochondrially encoded transcripts obtained from Ensembl\cite{cunningham_ensembl_2022}. As a control, we analysed nuclear-encoded transcripts localised to mitochondria, selecting top-scoring genes from the RNALocate database\cite{wu_rnalocate_2025}(Figure 3.17B). Among \gls{sg} datasets, two studies showed strong depletion of mitochondrial transcripts, whereas four displayed strong enrichment ($|NES| > 2$). No enrichment was observed for nuclear-encoded mitochondrial genes ($|NES| < 1$).
\\
\\
...
\\
\\
\gls{gsea} of this consensus \gls{sg} signal revealed enrichment (Figure 3.17C) of \gls{mtorc1} signalling and cholesterol homeostasis pathways, together with depletion of mitotic spindle transcripts, highlighting biological processes potentially associated with \gls{sg}-mediated transcript sequestration.
