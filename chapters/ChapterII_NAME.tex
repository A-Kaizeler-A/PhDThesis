...
\\
\\
Because the objective was to perform differential splicing analysis, only datasets with sufficient sequencing depth were considered (at least 5 \glspl{gb} of bases sequenced per sample, in all samples). High read depth is particularly important for splicing analyses. Accurate quantification of alternative splicing requires resolving the relative abundance of individual isoforms, including those expressed at low levels\cite{ascensao-ferreira_betas_2024}. Exon-exon junction reads, which are essential for detecting splice events, represent only a fraction of total \gls{rnaseq} reads, meaning that sufficient sequencing depth is required to accumulate enough junction-spanning reads for reliable quantification of alternative splicing events\cite{ascensao-ferreira_betas_2024}. Based on those criteria, we selected dataset GSE173953\cite{paget_stress_2023}, which includes \gls{u2os} and A549 \gls{g3bp1} \gls{wt} and \gls{ko} cells exposed to \gls{dsrna} as a proxy for viral infection. Importantly, \gls{g3bp1} knockout cells do not form \glspl{sg} under these conditions, enabling direct comparison between stressed cells with and without SG formation. This design allows the isolation of \gls{sg}-specific effects from general stress responses. Datasets GSE138988\cite{matheny_transcriptome-wide_2019}, and GSE119977\cite{matheny_rna_2021, matheny_corrigendum_2023} had the necessary sample types, but lower read coverage (less than 5 \glspl{gb} sequenced in any of the samples) than GSE173953\cite{paget_stress_2023} and were thus discarded.

\subsection{Correlations}

Unless otherwise indicated, the Spearman's correlation coefficient between t-statistics of differential gene expression and the associated test's p-value are shown in scatter plots.
...
\\
\\
Differential splicing analysis was then conducted using the betAS R package (v1.2.1)\cite{ascensao-ferreira_betas_2024}, with modifications to the default filtering procedure. By default, the \texttt{alternative\allowbreak Events} function excludes events in which at least one sample does not meet the \gls{psi} thresholds (1 $<=$ \gls{psi} $<=$ 99). However, applying these thresholds resulted in the inclusion of a large number of events that appeared constitutive, showing minimal variation rather than true alternative splicing.
\\
\\
...
Since a large number of transcriptomic datasets had been compiled for \glspl{sg}, we used alternative splicing as a proxy to assess the functional consequences of \gls{rbp} sequestration into \glspl{sg}. Changes in splicing are detectable at the transcriptomic level and provide a measurable readout of \gls{rbp} activity. If \gls{sg} sequestration affects splicing regulation, it is likely that other \gls{rbp}-dependent processes are also impacted.
\\
\\
Direct analysis of individual splicing events, however, is constrained by transcript expression, as alternative splicing can only be detected in expressed genes, and expression profiles vary substantially across cell types and conditions\cite{van_nostrand_large-scale_2020}. To address this, we adopted a complementary approach based on \gls{rbp} binding profiles. By grouping exon-skipping events according to their response to \gls{sg} assembly, increased inclusion, no significant change, or increased exclusion, we constructed \gls{rna} splicing maps, a widely used framework for identifying position-dependent \gls{rbp} binding signatures associated with specific regulatory outcomes\cite{van_nostrand_large-scale_2020}. Briefly, for each \gls{rbp}, its binding profile across a normalised metagene representation of the alternative exon and its flanking intronic and exonic regions is compared across the three event categories. Characteristic binding patterns, such as enrichment upstream of an exon in the increased inclusion category, can then be interpreted as indicative of a functional regulatory relationship between the \gls{rbp} and the splicing outcome. An illustrative example is provided in Figure 4.1, with the full methodological description, including the statistical framework used to assess differential binding at each position, is provided in Chapter 9.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{"images/Paper Images/RBFOX2"}
	\caption[RNA binding map of RBFOX2 in HepG2 and K562 cells exposed to shRNA against RBFOX2]{\textbf{\gls{rna} binding map of RBFOX2 in HepG2 and K562 cells exposed to \gls{shrna} against \gls{rbfox2}.}  (Top) Schematic of an alternative exon skipping event, regulated by an \gls{rbp}, \gls{rbfox2}. (Middle) Normalised binding of \gls{rbfox2} to metagenomic regulatory sequences of alternative exons. The plot discriminates excluded (blue), included (red) or maintained (black) alternative exons upon \gls{rbfox2} knockdown, obtained from \gls{encode}\cite{the_encode_project_consortium_integrated_2012, luo_new_2020}. The total number of events in each category can be found in brackets beneath the plot, preceded by the number of events for which there is at least one binding site for that \gls{rbp}. (Bottom) Chi-Squared Statistic of excluded (blue) or included (red) normalised binding, in relation to maintained events. Vertical lines delimitate the following metagenomic splicing regulatory sequences, in order: last 50 \glspl{nt} of upstream constitutive exon; first 200 \glspl{nt} of upstream intron; last 200 \glspl{nt} of upstream intron; first 50 \glspl{nt} of alternative exon; last 50 \glspl{nt} of alternative exon; first 200 \glspl{nt} of downstream intron; last 200 \glspl{nt} of downstream intron; first 50 \glspl{nt} of downstream constitutive exon. This example shows normalised \gls{rbfox2} binding enriched downstream of alternative exon. Upon \gls{rbfox2} knockdown, this binding is therefore lost, consistent with increased exon skipping. These results align with the known role of \gls{rbfox2} in promoting exon inclusion\cite{van_nostrand_large-scale_2020}, providing a proof of concept for this approach.}
	\label{fig:rbfox2}
\end{figure}
\noindent
Therefore, we compiled a set of \glspl{rbp} known to localise to \glspl{sg}\cite{nunes_msgp_2019, millar_new_2023} and mapped their \gls{rna} targets using \gls{eclip} data. We then performed differential alternative splicing analysis between \gls{dsrna}-stressed and unstressed cells. This analysis identified six \glspl{rbp} (\gls{tia1}, \gls{qki}, \gls{ddx3x}, \gls{rbm15}, \gls{pcbp1}, and \gls{ybx3}) with significant enrichment or depletion of targets amongst differentially spliced sequences.
\\
\\
To minimise confounding between putative SG sequestration and expression, we excluded \glspl{rbp} whose expression differed between stressed and unstressed conditions in both \gls{sg}-competent and \gls{sg}-deficient cells\footnote{More copies of a protein, are naturally expected to lead to increased function. It is plausible that SG assembly would lead to expression changes that impact splicing regulation. Nevertheless, our aim was to uncover if SG sequestration could directly impact RBP function, and, as such, expression driven splicing alterations were excluded. Obviously, other types of post-transcriptional alterations could occur, potentially changing splicing, such as RNA modifications, but, as we had no way to infer them, we could not control for them.} (Figure 4.2A). This filtering yielded four candidate \glspl{rbp} with alterations in binding targets compatible with lower protein availability: \gls{qki}, \gls{tia1}, \gls{pcbp1}, and \gls{ybx3}. 

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{"images/Paper Images/Figure9"}
...
\begin{figure}[t]
	...
	\ContinuedFloat
	\caption[]{\textbf{Alternative splicing alterations in SG competent and incompetent cells.} (A) Volcano plot of \gls{dgea} between stressed and unstressed \glspl{wc} (X-Axis -  Log2 Fold-Change; Y-Axis – B-Statistic) in SG competent (Left) and incompetent (Right) U2-OS and A549 cell lines subjected to dsRNA as a stressor\cite{paget_stress_2023}; genes as circles, with RBPs with significant enrichment or depletion targets amongst differentially spliced sequences labelled and in red. Vertical dashed lines correspond to -0.5 and 0.5 Log2\gls{fc}. Horizontal dashed line corresponds to B-statistic $=$ 0. Only genes with -2 $<$ Log2\gls{fc} $<$ 2 are plotted. (B-E) RNA binding maps of alternative exon splicing events in shRNA treated cells for PCBP1 (B), QKI (C), TIA1 (D), and YBX3 (E) obtained from ENCODE\cite{the_encode_project_consortium_integrated_2012} (Left), in SG competent cells (Middle) and in SG incompetent cells (Right). (Top) Normalised binding of RBP of interest to metagenomic regulatory sequences of alternative exons. The plot discriminates excluded (blue), included (red) or maintained (black) alternative exons upon RBP knockdown (Left) or upon dsRNA stress stimuli, in SG competent (Middle) and incompetent (Right) cells. The total number of events in each category can be found in brackets beneath the plot, preceded by the number of events for which there is at least one binding site for that RBP. (Bottom) Chi-Squared Statistic of excluded (blue) or included (red) normalised binding, in relation to maintained events. Vertical lines delimitate the following metagenomic splicing regulatory sequences, in order: last 50 nt of upstream constitutive exon; first 200 nt of upstream intron; last 200 nt of upstream intron; first 50 nt of alternative exon; last 50 nt of alternative exon; first 200 nt of downstream intron; last 200 nt of downstream intron; first 50 nt of downstream constitutive exon.}
\end{figure}
\noindent
To determine whether the observed splicing patterns were consistent with loss of RBP function, we compared these profiles against splicing changes induced by shRNA-mediated KD of each RBP (Figure 4.2B–E, Left). RBPs whose binding profiles resembled KD-induced splicing changes were considered candidates for functional impairment through SG sequestration (Figure 4.2B–E, Middle). Of the four candidates, only TIA1 and YBX3 showed meaningful similarity, with Pearson correlations of 0.44 and 0.77, respectively, between their binding profiles and KD-induced splicing changes in SG-harbouring conditions (Figure 4.2B–E, Middle).
\\
\\
However, these similarities could reflect general stress-induced splicing changes rather than SG-specific effects. To address this, we repeated the analysis in \gls{g3bp1}/\gls{g3bp2} \gls{ko} cells exposed to \gls{dsrna}, a condition in which cells are stressed but \glspl{sg} do not form\cite{paget_stress_2023} (Figure 4.2B–E, Right). Under these conditions, the correlation for YBX3 remained substantial (r=0.64), whereas that of TIA1 dropped markedly (r=0.07), suggesting that YBX3's splicing pattern reflects a general stress response independent of SG assembly, while TIA1's similarity to its KD profile appears to be specifically contingent on SG formation and thus consistent with functional impairment through SG sequestration.
\\
\\
To further investigate \gls{tia1} behaviour, we performed \gls{if} under basal and hyperosmotic stress conditions. Under basal conditions, \gls{tia1} is typically nuclear\cite{noauthor_human_nodate, thul_subcellular_2017}; however, in our experiments it appeared predominantly cytoplasmic. A possible explanation, is that this may reflect baseline stress associated with high cell confluency, which was maintained deliberately to counteract potential cell death under experimental stress conditions. Importantly, both control and stressed conditions were seeded at equal density to ensure comparability (Figure 4.3A). No clear differences in overall localisation were observed between conditions. Nevertheless, under hyperosmotic stress, \gls{tia1} consistently formed cytoplasmic foci compatible with putative \glspl{sg}\footnote{The identification of \glspl{sg} by immunofluorescence is inherently circular, as their definition relies on the visual detection of nucleating \glspl{rbp} such as \gls{tia1} or G3BP1. While all \glspl{sg} are expected to contain \gls{tia1}, not every \gls{tia1} focus necessarily constitutes a \gls{sg}, representing a fundamental limitation of morphology-based \gls{sg} identification that remains to be resolved in the field.}.
\\
\\
...
\\
\\
Importantly, gene expression (Figure 4.3B) and splicing (Figure 4.3C) changes following \gls{tia1} knockdown\cite{the_encode_project_consortium_integrated_2012} did not recapitulate those observed during \gls{sg} assembly (as Spearman's $\rho$ is very close to 0, suggesting there is no meaningful correlation) in \gls{dsrna}-treated cells\cite{paget_stress_2023}. This indicates that \gls{tia1} sequestration alone is unlikely to fully explain the broader transcriptomic effects associated with \gls{sg} formation.
 

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{"images/Paper Images/TESE_Fig5.3"}
...
\begin{figure}[t]
	\captionsetup{labelformat=adja-page}
	\ContinuedFloat
	\caption[]{\textbf{Alterations of TIA1 function upon KD and SG assembly.} (A) Immunofluorescence images of U2-OS cells untreated or treated with sorbitol (0.4M). TIA1 is shown in white and nuclei (DAPI) in blue. Yellow arrows highlight putative SGs. Scale bar is shown on bottom right corner of each image, in yellow. (B) Scatter plot of t-statistics of differential gene expression in HepG2 and K562 cells treated with shRNA against TIA1 and those with untargeted shRNA\cite{the_encode_project_consortium_integrated_2012} (X-Axis) and between U2-OS and A549 WT Cells (SG-Competent) exposed to dsRNA as a stressor against their G3BP1/G3BP2 \gls{ko} counterparts (SG-Incompetent)\cite{paget_stress_2023} (Y-Axis); genes as circles; density contour lines in dark red, Spearman's correlation coefficient $\rho$ and associated test's p-value on top left corner. (C) Scatter plot of \gls{dpsi} of differential splicing analysis in HepG2 and K562 cells treated with shRNA against TIA1 and those with untargeted shRNA\cite{the_encode_project_consortium_integrated_2012}(X-Axis) and between U2-OS and A549 WT Cells (SG-Competent) exposed to dsRNA as a stressor against their G3BP1/G3BP2 \gls{ko} counterparts (SG-Incompetent)\cite{paget_stress_2023} (Y-Axis); events as circles; density contour lines in dark red, Spearman's correlation coefficient $\rho$ and associated test's p-value on top left corner.}
\end{figure}
\noindent