...
\begin{enumerate}
	\item To define the \gls{sg}-associated transcriptome through meta-analysis, identifying transcripts preferentially sequestered within \glspl{sg};
	\item To characterise the transcriptomic landscape of \gls{sg}-harbouring cells, in order to determine which cellular pathways are altered upon \gls{sg} assembly;
	...
	\item To assess \glspl{sg} potential as therapeutic targets, apply the defined \gls{sg} transcriptomic signature to large-scale cancer cohorts, such as \gls{tcga}\cite{network_cancer_nodate, heath_nci_2021} to evaluate its prognostic value and association with patient survival.
	\comment{For Nuno: Achas que fica mais claro aqui a importância de saber se eles têm valor prognóstico? Há outras métricas para agressiveness, mas sinceramente, o end goal principal para mim é survival}
\end{enumerate}
\noindent
...
This thesis follows a coherent narrative centred on \gls{sg} biology, ending with some side projects that, while not directly part of the SG project, contributed essential technical and analytical expertise that strengthen the overall research project and are therefore presented as integral components of the doctoral training process. These included the entirety of Chapters 8 and 9, as well as Chapter 7.2.
\\
\\
...
\\
\\
Chapters 3 through 7 (with the exception of chapter 7.2 as mentioned above) address the specific aims described above and comprise the main body of the \gls{sg}-centred research. Each chapter contains an independent Methods section detailing the experimental and computational approaches relevant and contained to that section. For \textit{in vitro} experiments, in the interest of reproducibility, reagents \glspl{rrid} and \gls{cvcl} identifiers are provided. Chapter 3 includes an in-depth analysis of representative \gls{sg}-transcriptome datasets, serving as a methodological reference framework for subsequent analyses performed across additional datasets.
\\
\\
Chapter 10 provides a full list of used material, syntheses of the principal findings, discusses key limitations, and situates the results within the broader context of the \gls{sg} field, including ongoing debates and conceptual challenges. This chapter also includes critical self-reflection on the implications of the work and potential future directions. Additionally, it summarises collaborative projects conducted in parallel with the primary thesis work, reflecting the integrative and interdisciplinary nature of contemporary biomedical research.
