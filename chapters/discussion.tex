...
\subsection{Openness, Reproducibility, and the Importance of Critical Thinking}
...
\\
\\
Underlying both openness and reproducibility, however, is something more fundamental: critical thinking. Throughout this PhD, perhaps the most important skill I developed was not any particular analytical method, but the habit of interrogating my own results before accepting them. This manifested in concrete ways: questioning whether an observed signal could be explained by a technical artefact before attributing it to biology, and being willing to discard findings, including my own, when the evidence did not hold up to scrutiny. The centrifugation length bias, the Y-chromosome annotation errors, the irreproducible published count tables: none of these would have been identified without a disposition to question rather than blindly accept.
\\
\\
This is easier said than done. The natural inclination as humans is to find what we are looking for. A result that confirms a hypothesis feels satisfying; one that complicates or contradicts it feels like failure. But the most scientifically valuable moments of this PhD were often precisely the latter. One of my central goals was a \gls{sg} transcriptomic signature for application in cancer research. The apparent consistency across \gls{dc} datasets was initially encouraging, suggesting that \gls{sg}s — and by extension, any signature derived from them, were robustly reproducible. Discovering that this consistency most likely reflected a shared methodological bias rather than genuine biology was deeply uncomfortable. And yet it was the more honest result, and ultimately the more useful one: a signature that captured centrifugation physics rather than \gls{sg} biology would have been worthless as a cancer biomarker, and potentially dangerous. I should note, in the spirit of the same critical honesty, that the signature I ultimately derived is based on \gls{wc} transcriptomic signal rather than \gls{sg}-intrinsic content, which somewhat mitigates, though does not entirely resolve, this concern.
\\
\\
\comment{For Nuno: Added this section to talk about critical thinking, as I removed it from the CC section}Critical thinking also means holding two things simultaneously: confidence in a signal that is robust and reproducible, and humility about the limits of what that signal can support. The mitochondrial RNA enrichment in \glspl{sg} is, I believe, real. It survives methodological correction, appears across orthogonal approaches, and is corroborated by direct imaging. The mechanistic model proposed to explain it is considerably more speculative. It is internally consistent with all observations reported here, but direct experimental support currently rests on a single finding: that VDAC inhibition abrogates \gls{sg} assembly. That is a suggestive result, not a mechanistic proof. I have tried throughout this thesis to be explicit about that distinction. A PhD trains many things, but the most transferable of them is perhaps the discipline to know the difference between what your data shows and what you wish it showed. Perhaps most important of all, it is to not lose sight of doing good science in the pursuit of doing great science.
\\
\\
One might expect that, unlike wet lab protocols, computational analyses are largely invariant. After all, the choice between \texttt{edgeR} and \texttt{DESeq2} aside, most pipelines ask very similar questions of the same data. \comment{For Nuno: Aqui, referiste que nao tinha active verb. Fica melhor assim?}These methodological differences, should in principle should be negligible, equivalent to growing cells in a T-25 instead of a T-75. And yet, in practice, that is often not what we find. On numerous occasions, we were unable to replicate published results even when starting from the authors' own read count tables, setting aside any alignment differences entirely.
\\
\\
...
\\
\\
These resources will be made available upon publication through appropriate repositories (e.g., Zenodo). Until then, they are available upon reasonable request. This approach reflects a balance between openness and the practical considerations associated with ongoing work and data dissemination. 
\\
\\
...
\textbf{voyAGEr} was one such case. Reviewers correctly identified that GTEx harbours substantial batch effects leading to erroneous interpretations. Resolving these, and rebuilding the original \textbf{voyAGEr} to accommodate the corrections, was non-trivial. Together with colleague Rita Silva, we applied batch correction methods to address this, which is described in more detail in Chapter 7.2. This work resulted in a co-first author publication in \textit{eLife}\cite{schneider_voyager_2024}.
\\
\\
While validating the \gls{sg} Score, I encountered the problem that most gene signatures appear prognostic in cancer, yet no straightforward tool existed to assess this. At the time, Rita was developing \textbf{markeR}, a tool to evaluate how well a signature reflects a given phenotype. It was natural to add a function comparing signatures against randomly generated ones, as well as one to assess overlap with existing signatures. We implemented both together, resulting in an R package that is currently available as a preprint\cite{martins-silva_exploring_2025} and in press in NAR Genomics \& Bioinformatics.
...
To circumvent the limitations of single-gene approaches, we developed a machine learning-derived transcriptomic signature, the \gls{sg} Score, using elastic net regression on integrated datasets\cite{matheny_transcriptome-wide_2019, matheny_rna_2021, matheny_corrigendum_2023}. Following validation in an independent cohort\cite{paget_stress_2023} (Figure 8.3), the signature was applied across GDC-TCGA cancer datasets\cite{network_cancer_nodate, heath_nci_2021}. Contrary to the widely held assumption that \gls{sg} formation is globally elevated in cancer due to the prevalence of tumour-associated stress\cite{anderson_stress_2015}, elevated \gls{sg} Scores in tumour \gls{vs} normal tissue were observed in only three of seventeen cancer types with sufficient normal sample representation; seven cancer types showed the opposite pattern (Figure 8.4). This is consistent with experimental observations that robust \gls{sg} assembly requires stress conditions more severe than those typically encountered in physiological or even many tumour settings\cite{glauninger_stressful_2022}.
Survival analysis stratified by \gls{sg} Score, benchmarked against 100 randomly generated signatures to control for the well-documented tendency of gene signatures to show spurious prognostic value\cite{venet_most_2011, shimoni_association_2018}, identified two cancer types, ACC, and MESO, in which lower \gls{sg} Scores were associated with significantly poorer outcomes (Figure 8.5), contrary to to their previously described impact in tumour agressiveness\cite{somasekharan_yb-1_2015, fonteneau_stress_2022, grabocka_mutant_2016}.
\\
\\
The \gls{sg} Score, while showing promise as a prognostic biomarker, requires validation in prospective and independent cohorts before clinical utility can be established. Beyond its prognostic association, it may provide a quantitative readout of stress-response states linked to inflammatory regulation in tumours. In the two cancer types where prognostic significance was observed, higher \gls{sg} Scores were associated with improved survival and accompanied by elevated interferon and inflammatory signalling, suggesting that \gls{sg} assembly may reflect, or contribute to, a tumour microenvironment state that is less permissive to tumour progression. In this context, the score could potentially serve to stratify tumours according to their stress-adaptation capacity, which may in turn correlate with immune microenvironment composition and the degree of inflammatory infiltration.
In a translational setting, such a metric could be explored as a complementary biomarker for therapy response, particularly in treatments that engage cellular stress pathways, such as chemotherapy or radiotherapy, where \gls{sg} dynamics are expected to be relevant\cite{szaflarski_vinca_2016}. Counterintuitively, if higher \gls{sg} activity is associated with better outcomes, therapeutic strategies that enhance rather than suppress \gls{sg} assembly may warrant investigation in these tumour contexts. Longitudinal monitoring of the \gls{sg} Score during treatment could further provide a means to track adaptive stress responses in tumour cells. However, these applications remain speculative, and the mechanistic basis of the observed prognostic associations must be established before clinical translation can be considered.
\\
\\
A particularly notable finding was the upregulation of \textit{HK2} in \gls{sg}-high tumours (Figure 8.6C), a gene known to promote VDAC pore opening and facilitate ATP transport from mitochondria to the cytosol\cite{haloi_structural_2021}. Given that VDAC pores also serve as conduits for mitochondrial dsRNA efflux\cite{krieger_trafficking_2024}, this observation is consistent with our proposed model linking dsRNA release to \gls{sg} assembly and inflammation. We present this as a hypothesis rather than a conclusion, and note that no direct experimental validation was performed. We call attention to it here in the hope of guiding future studies.
...
\\
\\
In this sense, the present work delineates the broad outlines of the system: identifying consistent associations and hypothesising key axes of regulation, while the finer mechanistic detail remains to be resolved. We may have identified the continent and mapped the coastline; the interior remains to be explored.