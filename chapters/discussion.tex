...
As bioinformaticians and computational biologists, we are particularly invested in openness. Although some more orthodox practitioners also generate their own experimental data, as I did, with varying degrees of success, most of the data used in this thesis are publicly available. It is true that collaborations occasionally grant access to unpublished datasets, but open data remains the cornerstone. Without it, the meta-analysis of \gls{sg} content presented here would simply not have been possible. That said, mere availability is not enough: data must be properly annotated and described, which, sadly, was often not the case.
\\
\\
Open data can feel exposing. It opens the book on every corner of a project, inviting people like me to point out that your replicates cluster suspiciously\cite{matheny_rna_2021, matheny_corrigendum_2023}, or that your supposedly female samples express Y chromosome genes\cite{padron_proximity_2019}, which can be embarrassing. And yet, I firmly believe that bad data is better than no data at all. One might argue that bad data propagates bad conclusions downstream, such as the detection of neurodegeneration-associated transcripts, which happen to be longer than average, in \glspl{sg} being a case in point\cite{sato_stress_2023}. But without open data, that observation would likely have been made regardless, and bioinformaticians would have had no means to model the centrifugation artefact, nor to raise the points discussed throughout this thesis.
\\
\\
Reproducibility is equally critical. What is the value of a phenotype that only manifests under highly specific conditions? Only, as it were, when Mercury is in retrograde? An observation that holds across multiple methods, stressors, and cell types carries far greater weight (this pun was honestly not intentional, I only caught it while proofreading the thesis). The consistent enrichment of mitochondrial RNA within \glspl{sg}, shown here is replicable using orthogonal approaches, is a case in point. Open data further reinforces this: if an independent researcher, starting from the same raw data, reaches the same conclusions, the result is considerably more robust. In that sense, bioinformaticians and computational biologists are perhaps the ultimate peer reviewers.
...
\\
\\
I will be partially hypocritical here, however: not all data are hosted on GitHub. This is for practical reasons. The binding data, encompassing direct and indirect targets, is too large for GitHub's constraints, and a similar issue arose with the \gls{if} images, particularly the super-resolution ones. I expect both to be made available upon publication. Nevertheless, I reiterate the importance of openness. The corresponding author designation is too often treated as a mere box to be checked, much like open data policies themselves. I have sent many an email requesting data or clarification, receiving either vague replies or none at all. I cannot promise I will never find myself on the other side of that exchange, but I can commit that if data or questions are sent to me, I will make every effort to respond properly and accommodate the request.
\\
\\
In the interest of reproducibility, in addition to \textit{in silico} resources, we also provide a full list of \textit{in vitro} materials and reagents used, as well as their \glspl{rrid} and \gls{cvcl} identifiers, with the exception of those used throughout Chapter 3:
\begin{table}[H]
	\centering
...
As I implied above, bioinformaticians are too often treated as second-rate scientists, as a service rather than a discipline. The respect is rarely forthcoming, though I will acknowledge that finding anyone who feels their work is truly appreciated is a challenge regardless of field. The usual justification for this particular brand of dismissal is something along the lines of \textit{"you lot don't even know basic biology"}, presumably because many of us come from engineering backgrounds and therefore had fewer formal biology courses. I was, on multiple occasions, simply referred to as an engineer, despite being a biomedical scientist and oncobiologist by training. And even engineers, for what it is worth, have biology courses.
...
Several important caveats must be acknowledged. Our analysis did not distinguish between \gls{sg} subtypes, which differ in their dependence on \gls{eif2a} phosphorylation and can contribute to cross-dataset variability. The centrifugation correction, while effective in reducing length bias, rests on assumptions derived from yeast sedimentation data that may not fully generalise to human cells. Mitochondrial RNA enrichment was not observed in all datasets passing quality thresholds, partly a consequence of potential overcorrection, and our imaging experiments, though supportive of the dsRNA sequestration model, are limited by sample size and antibody non-specificity. The SG Score, while showing promise as a prognostic biomarker, requires validation in prospective and independent datasets before its clinical utility can be assessed.
\\
\\
The field would benefit from a second meta-analysis once transcriptomics studies employing more rigorous and standardised \gls{sg} isolation methods have accumulated. The mechanistic links between mitochondria, \gls{sg}s, and inflammatory signalling uncovered here, particularly the putative requirement for VDAC activity in \gls{sg} assembly, and the enrichment of \textit{HK2} in prognostically relevant tumours, necessitates direct experimental follow-up. We identified the continent and mapped the coastline; the interior remains to be explored.